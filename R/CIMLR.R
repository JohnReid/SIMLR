...
"CIMLR" <- function(X, c, no.dim = NA, k = 10, cores.ratio = 1) {
  ...
  numerator <- (apply(array(0, c(length(di[, k + 1]), dim(di)[2])), MARGIN = 2, FUN = function(x) {
    x <- di[, k + 1]
  }) - di)
  temp <- (k * di[, k + 1] - apply(di[, 1:k], MARGIN = 1, FUN = sum) + .Machine$double.eps)
  denominator <- apply(array(0, c(length(temp), dim(di)[2])), MARGIN = 2, FUN = function(x) {
    x <- temp
  })
  temp <- numerator / denominator
  a <- apply(array(0, c(length(t(1:num)), dim(di)[2])), MARGIN = 2, FUN = function(x) {
    x <- 1:num
  })
