...
"fast.pca" = function(X, K) {
  ...
  X = X - t(apply(array(0, dim(X)), MARGIN = 1, FUN = function(x) {
    x = tmp_val
  }))
  res = fast.rsvd(X, K)
  U = res$U
  ...
  X = X / apply(array(0, c(length(normalization_val), K)), MARGIN = 2, FUN = function(x) {
    x = normalization_val
  })
...
"multiple.kernel_large_scale" = function(val, ind, kk = 20) {
  ...
  for (l in 1:length(allk)) {
    if (allk_fun[l] < (ncol(val_fun))) {
      ...
      TT0 = apply(array(0, c(ncol(val_fun))), MARGIN = 1, FUN = function(x) {
        x = TT
      })
      ...
      for (j in 1:length(sigma_fun)) {
        ...
        temptemp = temp[, 1]
        temp = (apply(array(0, c(ncol(val_fun))), MARGIN = 1, FUN = function(x) {
          x = temptemp
        }) + matrix(temptemp[ind_fun], nrow = dim(TT0)[1], ncol = dim(TT0)[2])) * 0.5 - temp
        KK_fun = KK_fun + 1
