...
"multiple.kernel.numc" <- function(x, cores.ratio = 1) {
  ...
  D_Kernels <- unlist(parLapply(cl, 1:length(allk), fun = function(l, x_fun = x, Diff_sort_fun = Diff_sort, allk_fun = allk,
    ...
    if (allk_fun[l] < (nrow(x_fun) - 1)) {
      ...
      Sig <- apply(array(0, c(nrow(TT), ncol(TT))), MARGIN = 1, FUN = function(x) {
        x <- TT[, 1]
      })
      Sig <- Sig + t(Sig)
      Sig <- Sig / 2
  ...
  for (i in 1:length(D_Kernels)) {
    ...
    G <- K * (k %*% t(k))
    G1 <- apply(array(0, c(length(diag(G)), length(diag(G)))), MARGIN = 2, FUN = function(x) {
      x <- diag(G)
    })
    G2 <- t(G1)
...
"network.diffusion.numc" <- function(A, K) {
  ...
  W <- (W * (1 - diagonal_matrix)) / apply(array(0, c(nrow(W), ncol(W))), MARGIN = 2, FUN = function(x) {
    x <- (1 - diag(W))
  })
